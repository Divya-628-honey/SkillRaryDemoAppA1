...
public class CreateCourseTest extends BaseClass {
	...
	SoftAssert soft = new SoftAssert();
		...
		Map<String, String> map = excel.readFormExcel("Sheet1", "Add Course");
		String coueseName = map.get("Name")+jutil.generateRandomNum(100);
		addCourse.setName(coueseName);
		...
		for (WebElement name: courseNameList) {
			if(name.getText().equals(coueseName)) {
				isPresent = true;
				break;
			}
		}
			soft.assertTrue(isPresent);
		course.clickDeleteButton(coueseName, driver);
		course.clickDelete();
		soft.assertTrue(course.getSuccessMessage().contains("Success"));
